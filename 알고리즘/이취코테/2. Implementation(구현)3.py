...
- 입력조건
    ...
    - 둘째 줄: 캐릭터 좌표(A,B)와 바라보는 방향(d)가 공백을 기준으로 입력
...
maps = np.array([list(map(int, sys.stdin.readline().rstrip().split())) for _ in range(n)])
arrived[x][y] = 1

start_time = time.time()
wnd = {0:(-1,0), 1:(0,1), 2:(1,0), 3:(0,-1)}
# print(maps.T)
...
while way<4:
    ...
    a = x+wnd[z][0]
    b = y+wnd[z][1]
    if maps[a][b] == 0 and arrived[a][b] == 0: # 캐릭터가 보고있는 곳이 육지면서 안가본 곳이면
        cnt+=1   # 1칸 세기
        x = a
        y = b
        arrived[x][y] = 1
