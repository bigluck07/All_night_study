...
농장의 조감도는 아래와 같다. 
                   ...
                   |
                   6
헛간 4, 5, 6은 모두 2의 거리를 가진다. 
여기서 4번 헛간을 선택하는 이유는 헛간의 번호가 가장 작기 때문이다.
'''
...
steps = [float("inf") for _ in range(n+1)] # 1노드에서의 거리 측적을 위한 것
steps[1] = 0 # 1번은 0거리


graph = [[] * (n+1) for _ in range(n+1)] # 농장 그래프
for _ in range(m):
    a,b = map(int, sys.stdin.readline().strip().split())
    graph[a].append((1,b)) # 양방향으로 넣기(거리, 인접노드))
    graph[b].append((1,a))
...
while hq:
    ...
    if steps[v] > w: #짧으면 바꿔주기
        steps[v] = w
        for w2, v2 in graph[v]:
            heapq.heappush(hq, (w2+w, v2)) # 인접노드 큐에 넣기

m_num = max(steps[1:]) # 0번농장은 그냥 임의로 넣은거임
print(steps.index(m_num), end =" ")
print(m_num, end =" ")
print(steps.count(m_num))     
