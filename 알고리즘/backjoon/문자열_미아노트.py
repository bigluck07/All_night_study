...
평소와 다를 바 없이 과일들을 잔뜩 관찰하고 기쁜 마음으로 하교하던 어느 날, 
친구가 뒤에서 덮치는 바람에 실수로 비밀 노트를 물에 빠뜨리고 말았다.
...
입력
첫째 줄에 원래 문자열의 길이 N, 세로로 번진 글자의 개수 H, 가로로 번진 글자의 개수 W가 주어진다. 
...
for _ in range(h):
    ...
    for i in range(0,(n*w),w): # 번진 글자를 원래 글자수대로 쪼개기
        temp.append(row[i:point])
        point+=w
        
    for idx, i in enumerate(temp): # 원래 글자 자리 인덱스, 글자 뭉치
        state = True

        for j in i:
            if answer[idx] == '?': # 이미 글자 확인 됐는지 확인
                if j == '?': # 번진글자 확인 불가
                    pass
                else:        # 번진글자 확인 가능
                    answer[idx] = j
