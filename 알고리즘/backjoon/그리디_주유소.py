...
2 초	512 MB	46975	18056	14245	38.073%
문제
...
도로의 길이는 1이상 1,000,000,000 이하의 자연수이다. 
...
distnc = list(map(int, sys.stdin.readline().strip().split()))
oils = list(map(int, sys.stdin.readline().strip().split()))
oils = oils[:-1]+[0] # 마지막은 필요 없음

def solve(n,distnc, oils):
    start = oils[0]
    bill = 0
    point = 0
    for i in range(1, n):
        if start >= oils[i]:
            bill += (start * sum(distnc[point:i])) # start 보다 기름이 싸면 이후 거리는 그 도시로 start로 변경 
            start = oils[i]
            point = i
    return bill

print(solve(n, distnc, oils))